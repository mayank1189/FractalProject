...
bbbsDF.convert_objects(convert_numeric=True)
...
playermatchScoreDF=pd.read_csv('C://Users//Mayank//Desktop//Project-Fractal//Gen1//matchwiseplayerscore.csv')
...
#def getPlayerMatchScore(Match_Id,Team_Id):
#    print("match wise player data")
#    data=pd.DataFrame(columns=('Match_Id', 'Team_Id', 'Score'))
#    DF=pd.read_csv('C:\\Users\\Mayank\\Desktop\\Project-Fractal\\Gen1\\player_match_score1.csv')
#    df1=DF.groupby(['Match_Id','Team_Id'])['Score'].sum()
#    #df1.describe()
#    df1.to_csv("C:\\Users\\Mayank\\Desktop\\Project-Fractal\\Gen1\\matchwiseplayerscore.csv",sep=',')
#    return df1['Score']

def getPlayerMatchScore(team_id, opponent_id ):
    return playermatchScoreDF[(playermatchScoreDF['Team_Id']==team_id) & (playermatchScoreDF['Opponent_Id']==opponent_id)]

print(getPlayerMatchScore(1,2))

def getPlayingTeamPlayersScore(teamid):
    print(teamid)
    if(teamid==1):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//KKR.xlsx")
    if(teamid==2):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//RCB.xlsx")
    if(teamid==3):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//CSK.xlsx")
    if(teamid==4):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//KXIP.xlsx")
    if(teamid==5):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//RR.xlsx")
    if(teamid==6):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//DD.xlsx")
    if(teamid==7):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//MI.xlsx")
    if(teamid==8):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//DC.xlsx")
    if(teamid==9):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//KTK.xlsx")
    if(teamid==10):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//PW.xlsx")
    if(teamid==11):
        teamIdDF=pd.read_excel("C://Users//Mayank//Desktop//Project-Fractal//data//teamplayers//SRH.xlsx")
    playingX11Score=teamIdDF['Score'].sum()
    #opponentIdScorePlayingX11=opponentIdDF['Total_Score'].sum()
#    data.loc[pos,"Team_Id"] = teamIdDF
#    data.loc[pos,"Opponent_Id"] = opponentIdDF
#    data.loc[pos,"Team_Id_PX11Score"] = teamIdScorePlayingX11
#    data.loc[pos,"Opponent_Id_PX11Score"] = opponentIdScorePlayingX11
    return playingX11Score
...
def generatePredictData(teamId,opponentId,tossWon,batFirst,venueId, day):    
    data = pd.DataFrame(columns=('Match_Id','Team_Id','Day','Toss_Won','Bat_First','Venue_Id','Win_Percenetage','Opponent_Win_Percentage','Win_Percenetage_Against','Toss_Decision_Win_Percentage','Bat_Decision_Win_Percentage','Team1_Score','OpponentId_Score','Match_Won'))
    pos = 0
    matchId = 1
    teamTossWon = 0
    teamBatFirst = 0
    #PlayerScoreDF=getPlayerMatchScore(teamId, opponentId)
    #teamIdScore=PlayerScoreDF['Score_Team1'].sum()
    #opponentIdScore=PlayerScoreDF['Score_OpponentTeam'].sum()
    teamWinPercentage = getTeamWinPercentage(teamId)
   ...
   # data.loc[pos,"Venue"] = venue
    ...
    data.loc[pos,"Toss_Won"] = teamTossWon
    data.loc[pos,"Bat_First"] = teamBatFirst
    data.loc[pos,"Venue_Id"] = venueId
    data.loc[pos,"Win_Percenetage"] = teamWinPercentage
    data.loc[pos,"Opponent_Win_Percentage"] = opponentTeamWinPercentage
    data.loc[pos,"Win_Percenetage_Against"] = teamMatchWinPercentAgainstOpponent
    data.loc[pos,"Toss_Decision_Win_Percentage"] = teamTossDescionWinPercentage
    data.loc[pos,"Bat_Decision_Win_Percentage"] = teamBatDecisionWonPercentage
    data.loc[pos,"Team1_Score"] = getPlayingTeamPlayersScore(teamId)
    data.loc[pos,"OpponentId_Score"] = getPlayingTeamPlayersScore(opponentId)
...
generatePredictData(1,3,1,1,1,1)